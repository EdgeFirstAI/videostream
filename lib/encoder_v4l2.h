...
struct vsl_encoder_v4l2 {
    ...
    // Crop window into a larger source frame. When .valid, the encoder
    // OUTPUT queue is sized to the crop dimensions but bytesperline is the
    // source's full stride, and each QBUF sets data_offset so the hardware
    // reads only the cropped window.
    //
    ...
    // first vsl_encode_frame call. The position (crop.x, crop.y) can vary
    // per call — only data_offset/bytesused are recomputed each frame.
    // Subsequent calls that change the latched dimensions return -EINVAL.
    struct {
        bool valid;
        int  width;
        int  height;
