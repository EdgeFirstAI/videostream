...
// DESCRIPTION:
//   Tests VPU H.264 encoder keyframe insertion with 4K tiled encoding.
//   Validates keyframe detection and insertion logic.
...
encodeAndSave(void* arg)
{
    ...
    if (keyframe) {
        ...
        snprintf(buf,
                 100,
                 "/tmp/vslencodedvideo_tile%d-%d.hevc",
                 tile_num,
                 count[tile_num]);
        count[tile_num]++;
        printf("Changing vid files to %s\n", buf);
        close(tiles_fd[tile_num]);
        tiles_fd[tile_num] =
            open(buf, O_WRONLY | O_CREAT | O_TRUNC | O_SYNC, 0644);
    }
    ssize_t bytesWritten = write(tiles_fd[tile_num],
...
main(int argc, char* argv[])
{
    ...
    char buf[100];
    for (int i = 0; i < 4; i++) {
        snprintf(buf, 100, "/tmp/vslencodedvideo_tile%d-%d.hevc", i, count[i]);
        tiles_fd[i] = open(buf, O_WRONLY | O_CREAT | O_TRUNC | O_SYNC, 0644);
        count[i]++;
    }
