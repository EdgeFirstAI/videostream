...
static void VPU_EncSetEncInParamsEncode (VCEncIn *pEncIn, FRAME* frame, STREAM_BUFFER* stream, CONFIG* params, VCEncPictureCodingType nextCodingType)
{
  ...
  // In case frame->fb_bus_address_ChromaU and frame->fb_bus_address_ChromaV are null calculate it manually anyway
  pEncIn->busLuma = frame->fb_bus_address_Luma;

#ifndef ANDROID
  if(frame->fb_bus_address_ChromaU == NULL && frame->fb_bus_address_ChromaV == NULL)
  {
#endif
...
VpuEncRetCode VPU_EncEncodeFrame(VpuEncHandle InHandle, VpuEncEncParam* pInOutParam)
{
  ...
  ENCODER* this = (ENCODER*) pObj->codec;
  if (this->nextCodingType == VCENC_INTRA_FRAME) {
    pInOutParam->eOutRetCode |= VPU_ENC_OUTPUT_KEYFRAME;
  }
...
VpuEncRetCode VPU_EncOpen(VpuEncHandle *pOutHandle, VpuMemInfo* pInMemInfo,VpuEncOpenParam* pInParam)
{
  ...
  switch (pInParam->eFormat) {
    ...
    case VPU_V_HEVC:
      ...
      pObj->config.preProcCfg.xOffset = pInParam->nXOffset;
      pObj->config.preProcCfg.yOffset = pInParam->nYOffset;
...
VpuEncRetCode VPU_EncOpenSimp(VpuEncHandle *pOutHandle, VpuMemInfo* pInMemInfo,VpuEncOpenParamSimp * pInParam)
{
  ...
  sEncOpenParamMore.nXOffset = pInParam->nXOffset;
  sEncOpenParamMore.nYOffset = pInParam->nYOffset;
