 ...
 *
 *	History :
 *	Date	(y.m.d)		Author			Version
 *Description 2016-12-20		Song Bing		0.1
 *Created
 */
...
static int g_seek_dump =
    DUMP_ALL_DATA; /*0: only dump data after seeking; otherwise: dump all data*/

#define VPU_LOG(...) \
    if (nVpuLogLevel & 0x1) { LOG_PRINTF(__VA_ARGS__); }
#define VPU_TRACE
#define VPU_API(...) \
    if (nVpuLogLevel & 0x1) { LOG_PRINTF(__VA_ARGS__); }
#define VPU_ERROR(...) \
...
#define VPUWRAPPER_BINARY_VERSION_STR                          \
    (BASELINE_SHORT_NAME                     OS_NAME SEPARATOR \
     "Build on" SEPARATOR __DATE__ SEPARATOR __TIME__)

#define VPU_MEM_ALIGN 0x10

#define VPU_BITS_BUF_SIZE \
    (16 * 1024 *          \
     1024) // bitstream buffer size : big enough contain two big frames

#define VC1_MAX_SEQ_HEADER_SIZE \
    256 // for clip: WVC1_stress_a0_stress06.wmv, its header length = 176 (>128)
...
#define ALIGN(ptr, align)                                                    \
    ((align) ? ((((unsigned long) (ptr)) + (align) - 1) / (align) * (align)) \
             : ((unsigned long) (ptr)))
#define MemAlign(mem, align) ((((unsigned int) mem) % (align)) == 0)
#define MemNotAlign(mem, align) ((((unsigned int) mem) % (align)) != 0)

#define NotEnoughInitData(free) \
    (((VPU_BITS_BUF_SIZE) - (free)) < (VPU_MIN_INIT_SIZE))
#define NotEnoughDecData(free, min_validsize) \
    (((VPU_BITS_BUF_SIZE) - (free)) < (min_validsize))
...
typedef struct {
    ...
    int streamBufDelaySize; /*unit: bytes. used in stream mode:  valid data size
                               should reach the threshold before decoding*/
    ...
    int frameBufState[VPU_MAX_FRAME_INDEX]; /*record frame state for clearing
                                               display frame(if user forgot to
                                               clear them)*/
    ...
    int nAccumulatedConsumedStufferBytes; /*stuffer size between frames: if it
                                             <0, indicate that some frames are
                                             contained in config data*/
    int nAccumulatedConsumedFrmBytes;     /*frame size: >=0*/
    int nAccumulatedConsumedBytes; /*it should match with the input data size ==
                                      nAccumulatedConsumedStufferBytes+nAccumulatedConsumedFrmBytes*/
    VpuFrameBuffer* pLastDecodedFrm; /*the nearest decoded frame*/
    int nAdditionalSeqBytes; /*seq header inserted by wrapper itself , or config
                                data */
    int nAdditionalFrmHeaderBytes; /*frame header inserted by wrapper itself */
    unsigned int
        nLastFrameEndPosPhy; /*point to the previous frame tail: used to compute
                                the stuff data length between frames*/

    int nDecResolutionChangeEnabled; /*1: support resolution change
                                        notification; 0: not support*/
...
int
VpuLogLevelParse(int* pLogLevel)
{
    int   level = 0;
    FILE* fpVpuLog;
...
VPU_DecLoad()
{
    if (HantroDec_OpenLib() < 0) { return VPU_ENC_RET_INVALID_PARAM; }
    if (Codec_OpenLib() < 0) { return VPU_ENC_RET_INVALID_PARAM; }
...
VpuDecRetCode
VPU_DecGetVersionInfo(VpuVersionInfo* pOutVerInfo)
{
    if (pOutVerInfo == NULL) {
...
VpuDecRetCode
VPU_DecGetWrapperVersionInfo(VpuWrapperVersionInfo* pOutVerInfo)
{
...
VpuDecRetCode
VPU_DecQueryMem(VpuMemInfo* pOutMemInfo)
{
...
VpuDecRetCode
VPU_DecOpen(VpuDecHandle*    pOutHandle,
            ...
            VpuMemInfo*      pInMemInfo)
{
    ...
    if ((pMemVirt->pVirtAddr == NULL) ||
        MemNotAlign(pMemVirt->pVirtAddr, VPU_MEM_ALIGN) ||
        (pMemVirt->nSize != sizeof(VpuDecHandleInternal))) {
        VPU_ERROR("%s: failure: invalid parameter ! \r\n", __FUNCTION__);
        return VPU_DEC_RET_INVALID_PARAM;
    }

    if ((pMemPhy->pVirtAddr == NULL) ||
        MemNotAlign(pMemPhy->pVirtAddr, VPU_MEM_ALIGN) ||
        (pMemPhy->pPhyAddr == NULL) ||
        MemNotAlign(pMemPhy->pPhyAddr, VPU_MEM_ALIGN) ||
    ...
    if (pInParam->CodecFormat == VPU_V_HEVC ||
        pInParam->CodecFormat == VPU_V_VP9) {
    ...
    case VPU_V_AVC:
        if (pObj->config.g1_conf.bEnableTiled) pObj->config_tile = true;
        pObj->codec = HantroHwDecOmx_decoder_create_h264(pObj->pdwl,
                                                         bIsMvcStream,
                                                         &pObj->config.g1_conf);
        if (pObj->bReorderDisable &&
            pObj->codec->setnoreorder(pObj->codec, OMX_TRUE) != CODEC_OK) {
    ...
    case VPU_V_MPEG2: /**< AKA: H.262 */
        pObj->codec =
            HantroHwDecOmx_decoder_create_mpeg2(pObj->pdwl,
                                                &pObj->config.g1_conf);
        VPU_LOG("open Mpeg2 \r\n");
        break;
    case VPU_V_H263: /**< H.263 */
        pObj->codec =
            HantroHwDecOmx_decoder_create_mpeg4(pObj->pdwl,
                                                bDeblock,
                                                MPEG4FORMAT_H263,
                                                &pObj->config.g1_conf);
        VPU_LOG("open H263 \r\n");
    ...
    case VPU_V_VP8:
        ...
        break;
    case VPU_V_HEVC:
        if (pObj->config.g2_conf.bEnableTiled) pObj->config_tile = true;
        if (!pObj->bSecureMode) {
    ...
    // case VPU_V_VP9:
    //     if (pObj->config.g2_conf.bEnableTiled) pObj->config_tile = true;
    //     /* NVJM850-66: support vp9 resolution change to pass cts
    //      * testVP9_adaptiveSmallDrc */
...
VpuDecRetCode
VPU_DecGetCapability(VpuDecHandle     InHandle,
...
VpuDecRetCode
VPU_DecDisCapability(VpuDecHandle InHandle, VpuDecCapability eInCapability)
{
    VpuDecHandleInternal* pVpuObj = NULL;
    VpuDecObj*            pObj    = NULL;

    if (InHandle == NULL) { return VPU_DEC_RET_INVALID_PARAM; }
...
VpuDecRetCode
VPU_DecConfig(VpuDecHandle InHandle, VpuDecConfig InDecConf, void* pInParam)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
    int                   para;
    ...
    case VPU_DEC_CONF_INPUTTYPE:
        para = *((int*) pInParam);
        if ((para != VPU_DEC_IN_NORMAL) && (para != VPU_DEC_IN_KICK) &&
            (para != VPU_DEC_IN_DRAIN)) {
    ...
    case VPU_DEC_CONF_INIT_CNT_THRESHOLD:
        para = *((int*) pInParam);
        if (para <= 0) { return VPU_DEC_RET_INVALID_PARAM; }
    ...
    case VPU_DEC_CONF_ENABLE_TILED:
        if (pObj->CodecFormat == VPU_V_HEVC || pObj->CodecFormat == VPU_V_VP9) {
            ...
            if (pObj->CodecFormat == VPU_V_AVC) {
                pObj->config_tile = false;
                if ((*((int*) pInParam)) == 1) { pObj->config_tile = true; }
            }
        }
...
static void
WrapperFileDumpBitstrem(unsigned char* pBits, unsigned int nSize)
{
    int nWriteSize = 0;
    if (nSize == 0) { return; }
...
static void
WrapperFileDumpYUV(VpuDecObj* pObj, VpuFrameBuffer* pDisplayBuf)
{
    static int cnt        = 0;
    int        nCScale    = 1;
    int        nWriteSize = 0;
#define FRAME_ALIGN (16)
#define Alignment(ptr, align) \
            ...
            bool           useRingBuffer)
{
    ...
    if (!useRingBuffer) {
        ...
        if (VPU_DUMP_RAW) WrapperFileDumpBitstrem(pIn, len);
        return;
    }

    if (pObj->nBsBufOffset + pObj->nBsBufLen + len > VPU_BITS_BUF_SIZE) {
        if (pObj->ringbuffer) {
            memcpy(pObj->pBsBufVirtStart + pObj->nBsBufOffset + pObj->nBsBufLen,
                   pIn,
                   VPU_BITS_BUF_SIZE - pObj->nBsBufOffset - pObj->nBsBufLen);
            memcpy(pObj->pBsBufVirtStart,
                   pIn + VPU_BITS_BUF_SIZE - pObj->nBsBufOffset -
                       pObj->nBsBufLen,
                   len + pObj->nBsBufOffset + pObj->nBsBufLen -
                       VPU_BITS_BUF_SIZE);
        } else {
            if (pObj->nBsBufOffset)
                memmove(pObj->pBsBufVirtStart,
                        pObj->pBsBufVirtStart + pObj->nBsBufOffset,
                        pObj->nBsBufLen);
            pObj->nBsBufOffset = 0;
            memcpy(pObj->pBsBufVirtStart + pObj->nBsBufOffset + pObj->nBsBufLen,
                   pIn,
...
static VpuDecRetCode
VPU_DecProcessInBuf(VpuDecObj* pObj, VpuBufferNode* pInData)
{
    ...
    if (pObj->nPrivateSeqHeaderInserted == 0) {
        ...
        if (pObj->CodecFormat == VPU_V_DIVX3) {
            ...
            char format_[]   = "VP8 ";
            u8   tmp[4];
            if (pInData->nSize < 20) return VPU_DEC_RET_SUCCESS;
        ...
        if (0 != pInData->sCodecData.nSize) {
            if ((pObj->CodecFormat == VPU_V_AVC ||
                 pObj->CodecFormat == VPU_V_HEVC) &&
                (0 == pObj->nIsAvcc)) {
                if (pObj->CodecFormat == VPU_V_AVC)
                    VpuDetectAvcc(pInData->sCodecData.pData,
                                  pInData->sCodecData.nSize,
                                  &pObj->nIsAvcc,
                                  &pObj->nNalSizeLen,
                                  &pObj->nNalNum);
                else
                    VpuDetectHvcc(pInData->sCodecData.pData,
                                  pInData->sCodecData.nSize,
                                  &pObj->nIsAvcc,
                                  &pObj->nNalSizeLen,
                                  &pObj->nNalNum);
            ...
            if (pObj->nIsAvcc) {
                if (pObj->CodecFormat == VPU_V_AVC)
                    VpuConvertAvccHeader(pInData->sCodecData.pData,
                                         pInData->sCodecData.nSize,
                                         &pHeader,
                                         &headerLen);
                else
                    VpuConvertHvccHeader(pInData->sCodecData.pData,
                                         pInData->sCodecData.nSize,
                                         &pHeader,
                                         &headerLen);
                ...
                if ((pInData->pVirAddr == NULL) || (pInData->nSize < 4)) {
                    ...
                    // TODO: or define one default value when pInData->pVirAddr
                    // is NULL
                ...
                headerAllocated = 1;
                VC1CreateNALSeqHeader(pHeader,
                                      (int*) (&headerLen),
                                      pInData->sCodecData.pData,
                                      (int) pInData->sCodecData.nSize,
                                      (unsigned int*) pInData->pVirAddr,
            ...
            if (headerAllocated) { free(pHeader); }
        }
    ...
    if (pObj->nIsAvcc) {
        ...
        VpuConvertAvccFrame(pInData->pVirAddr,
                            pInData->nSize,
                            pObj->nNalSizeLen,
                            &pFrm,
                            &nFrmSize,
...
static int
VpuSearchFrameIndex(VpuDecObj* pObj, unsigned char* pInPhysY)
{
    int index;
    int i;
...
static VpuDecRetCode
VPU_DecGetFrame(VpuDecObj* pObj, int* pOutBufRetCode)
{
    ...
    case CODEC_HAS_FRAME:
        ...
        if (-1 == index) {
            VPU_ERROR("%s: failure: vpu can not find the frame buf, "
                      "pInFrameBuf=0x%X \r\n",
                      __FUNCTION__,
                      (unsigned int) frm.fb_bus_address);
            return VPU_DEC_RET_INVALID_PARAM;
        }
        ...
        pObj->frameInfo.pExtInfo->FrmCropRect.nRight =
            frm.outBufPrivate.nFrameWidth;
        pObj->frameInfo.pExtInfo->FrmCropRect.nBottom =
            frm.outBufPrivate.nFrameHeight;
        // pObj->frameInfo.pExtInfo->nQ16ShiftWidthDivHeightRatio=pSrcInfo->Q16ShiftWidthDivHeightRatio;
        if (frm.outBufPrivate.sRfcTable.nLumaBusAddress &&
            frm.outBufPrivate.sRfcTable.nChromaBusAddress) {
            pObj->frameInfo.pExtInfo->rfc_luma_offset =
                frm.outBufPrivate.sRfcTable.nLumaBusAddress -
                frm.fb_bus_address;
            pObj->frameInfo.pExtInfo->rfc_chroma_offset =
                frm.outBufPrivate.sRfcTable.nChromaBusAddress -
                frm.fb_bus_address;
        }
        VPU_LOG("crop: %d %d\n",
                (int) frm.outBufPrivate.nFrameWidth,
                (int) frm.outBufPrivate.nFrameHeight);
        VPU_LOG("video frame base: %p: RFC table Luma: %p Chroma: %p\n",
                (void*) frm.fb_bus_address,
                (void*) frm.outBufPrivate.sRfcTable.nLumaBusAddress,
                (void*) frm.outBufPrivate.sRfcTable.nChromaBusAddress);
...
static VpuDecRetCode
VPU_DecDecode(VpuDecObj* pObj, int* pOutBufRetCode)
{
    while (pObj->nBsBufLen > 0) {
        ...
        stream.bus_address =
            (OSAL_BUS_WIDTH) pObj->pBsBufPhyStart + pObj->nBsBufOffset;
        stream.streamlen = pObj->nBsBufLen;
        stream.allocsize = pObj->bSecureMode ? pObj->nSecureBufferAllocSize
                                             : VPU_BITS_BUF_SIZE;
        ...
        if (ret == -1 || first == last) {
            if (!pObj->eosing) break;
            // assume that remaining data contains a single complete decoding
            // unit fingers crossed..
            first = 0;
        ...
        if (0) // pObj->nBsBufOffset + first > 0)
        {
            ...
            memmove(pObj->pBsBufVirtStart,
                    pObj->pBsBufVirtStart + pObj->nBsBufOffset + first,
                    pObj->nBsBufLen);
        ...
        stream.bus_address =
            (OSAL_BUS_WIDTH) pObj->pBsBufPhyStart + pObj->nBsBufOffset + first;
    ...
    printf ("\n");
        ...
        VPU_LOG("stream.pBsBufPhyStart = %p,offset=%d",
                (char*) pObj->pBsBufPhyStart,
                pObj->nBsBufOffset);
...
static VpuDecRetCode
RvParseHeader(VpuDecObj* pObj, VpuBufferNode* pInData)
{
    u32           tmp, length;
    u8*           buff;
    ...
    if (!pObj->codec) {
        ...
        if (pInData->sCodecData.nSize < 20) return VPU_DEC_RET_SUCCESS;
        ...
        pObj->codec = HantroHwDecOmx_decoder_create_rv(pObj->pdwl,
                                                       bIsRV8,
                                                       size[num_frame_sizes],
                                                       frame_sizes,
                                                       nPicWidth,
                                                       nPicHeight,
                                                       &pObj->config.g1_conf);
    ...
    for (i = 0; i < pObj->slice_info_num; i++) {
        buff += 4;
        pObj->slice_info[i].is_valid =
            (buff[0] << 24) | (buff[1] << 16) | (buff[2] << 8) | (buff[3] << 0);
        buff += 4;
        pObj->slice_info[i].offset =
            (buff[0] << 24) | (buff[1] << 16) | (buff[2] << 8) | (buff[3] << 0);
    }
...
VpuDecRetCode
VPU_DecDecodeBuf(VpuDecHandle   InHandle,
                 ...
                 int*           pOutBufRetCode)
{
    ...
    *pOutBufRetCode = 0;
    ...
    printf ("\n");
        ...
        if (pObj->bSecureMode) pObj->nBsBufOffset = 0;
...
VpuDecRetCode
VPU_DecGetInitialInfo(VpuDecHandle InHandle, VpuDecInitInfo* pOutInitInfo)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
    STREAM_INFO           info;
    ...
    if (info.hdr10_available) {
        ...
        pOutInitInfo->Hdr10Meta.redPrimary[0] =
            info.hdr10_metadata.redPrimary[0];
        pOutInitInfo->Hdr10Meta.redPrimary[1] =
            info.hdr10_metadata.redPrimary[1];
        pOutInitInfo->Hdr10Meta.greenPrimary[0] =
            info.hdr10_metadata.greenPrimary[0];
        pOutInitInfo->Hdr10Meta.greenPrimary[1] =
            info.hdr10_metadata.greenPrimary[1];
        pOutInitInfo->Hdr10Meta.bluePrimary[0] =
            info.hdr10_metadata.bluePrimary[0];
        pOutInitInfo->Hdr10Meta.bluePrimary[1] =
            info.hdr10_metadata.bluePrimary[1];
        pOutInitInfo->Hdr10Meta.whitePoint[0] =
            info.hdr10_metadata.whitePoint[0];
        pOutInitInfo->Hdr10Meta.whitePoint[1] =
            info.hdr10_metadata.whitePoint[1];
        pOutInitInfo->Hdr10Meta.maxMasteringLuminance =
            info.hdr10_metadata.maxMasteringLuminance;
        pOutInitInfo->Hdr10Meta.minMasteringLuminance =
            info.hdr10_metadata.minMasteringLuminance;
        pOutInitInfo->Hdr10Meta.maxContentLightLevel =
            info.hdr10_metadata.maxContentLightLevel;
        pOutInitInfo->Hdr10Meta.maxFrameAverageLightLevel =
            info.hdr10_metadata.maxFrameAverageLightLevel;
    } else {
    ...
    if (info.colour_desc_available) {
        ...
        pOutInitInfo->ColourDesc.transferCharacteristics =
    ...
    if (info.chroma_loc_info_available) {
        pOutInitInfo->ChromaLocInfo.chromaSampleLocTypeTopField =
            info.chroma_sample_loc_type_top_field;
        pOutInitInfo->ChromaLocInfo.chromaSampleLocTypeBottomField =
    ...
    VPU_LOG("%s: min frame count: %d \r\n",
            __FUNCTION__,
            pOutInitInfo->nMinFrameBufferCount);
#ifndef NDEBUG
    printf("%s: buffer resolution: %lux%lu image: %lux%lu crop: %lu %lu %lu %lu \r\n",
            __FUNCTION__,
            info.stride,
            info.sliceheight,
            info.width,
...
VpuDecRetCode
VPU_DecRegisterFrameBuffer(VpuDecHandle    InHandle,
                           ...
                           int             nNum)
{
    ...
    for (i = targetNum; i < targetNum + nNum; i++) {
        ...
        VPU_LOG("%s: register frame index: %d virt: %p phy: %p ion fd %d\r\n",
                __FUNCTION__,
                i,
                pInFrameBufArray->pbufVirtY,
                pInFrameBufArray->pbufY,
                pInFrameBufArray->nIonFd);
...
VpuDecRetCode
VPU_DecGetOutputFrame(VpuDecHandle InHandle, VpuDecOutFrameInfo* pOutFrameInfo)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
...
VpuDecRetCode
VPU_DecGetConsumedFrameInfo(VpuDecHandle           InHandle,
                            VpuDecFrameLengthInfo* pOutFrameLengthInfo)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
    ...
    pOutFrameLengthInfo->nStuffLength =
        pVpuObj->obj.nAccumulatedConsumedStufferBytes;
    pOutFrameLengthInfo->nFrameLength =
        pVpuObj->obj.nAccumulatedConsumedFrmBytes;
...
VpuDecRetCode
VPU_DecOutFrameDisplayed(VpuDecHandle InHandle, VpuFrameBuffer* pInFrameBuf)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
    BUFFER                buff;
...
VpuDecRetCode
VPU_DecFlushAll(VpuDecHandle InHandle)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
    BUFFER                buff;
    ...
    do {
        ...
        if (OutBufRetCode & VPU_DEC_OUTPUT_DIS) {
            ...
            buff.bus_address =
                (OSAL_BUS_WIDTH) pVpuObj->obj.frameInfo.pDisplayFrameBuf->pbufY;
            pObj->codec->pictureconsumed(pObj->codec, &buff);
            pObj->nOutFrameCount--;
        }
...
VpuDecRetCode
VPU_DecAllRegFrameInfo(VpuDecHandle     InHandle,
                       VpuFrameBuffer** ppOutFrameBuf,
                       int*             pOutNum)
{
    VpuDecHandleInternal* pVpuObj;
    int                   i;
...
VpuDecRetCode
VPU_DecGetNumAvailableFrameBuffers(VpuDecHandle InHandle, int* pOutBufNum)
{
    int                   i, cnt;
    VpuDecHandleInternal* pVpuObj;
    pVpuObj = (VpuDecHandleInternal*) InHandle;
    ...
    cnt = 0;
    for (i = 0; i < pVpuObj->obj.frameNum; i++) {
        if (pVpuObj->obj.frameBufState[i] == VPU_FRAME_STATE_FREE) { cnt++; }
    }
    *pOutBufNum = cnt;
    return VPU_DEC_RET_SUCCESS;
}
...
VpuDecRetCode
VPU_DecClose(VpuDecHandle InHandle)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
    ...
    if (pObj->codec) pObj->codec->destroy(pObj->codec);

    if (pObj->pdwl) DWLRelease(pObj->pdwl);
    pObj->pdwl = NULL;
...
VpuDecRetCode
VPU_DecReset(VpuDecHandle InHandle)
{
    VpuDecHandleInternal* pVpuObj;
    VpuDecObj*            pObj;
...
VpuDecRetCode
VPU_DecGetErrInfo(VpuDecHandle InHandle, VpuDecErrInfo* pErrInfo)
{
...
VpuDecRetCode
VPU_DecGetMem(VpuMemDesc* pInOutMem)
{
    ...
    if (ret < 0) { return VPU_DEC_RET_FAILURE; }
    ...
    if (pdwl) DWLRelease(pdwl);

    return VPU_DEC_RET_SUCCESS;
}

VpuDecRetCode
VPU_DecFreeMem(VpuMemDesc* pInMem)
{
    struct DWLLinearMem info;
    struct DWLInitParam dwlInit;
    const void*         pdwl = NULL;
